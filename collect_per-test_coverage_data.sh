...
for testMethod in `cat coverage_data/list_of_test_methods.txt`; do
	...
	ack --noheading "id=\"L" --noignore-dir=target target | egrep -i "class=\"(pc|fc)" | sed 's/\.java.html:/-/' | sed 's#target/##' | sed 's#site/jacoco-ut/##' | sed 's#/#.#g' | sed 's/id=.*//' | sed 's/<span class=//' | sed 's/\"//g' > $op_file_name
